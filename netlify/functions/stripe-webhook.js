...
async function authenticate() {
  ...
  if (!data.result?.uid) throw new Error("Odoo 登录失败");
  // 返回 uid 和 session cookie
  const cookie = res.headers.get("set-cookie") || "";
  return { uid: data.result.uid, cookie };
}

async function execute(cookie, model, method, args, kwargs = {}) {
...
async function findOrCreatePartner(cookie, email, name) {
  const ids = await execute(cookie, "res.partner", "search", [[["email", "=", email]]]);
  if (ids.length > 0) return ids[0];
  const id = await execute(cookie, "res.partner", "create", [{
    name: name || email,
    email: email,
    customer_rank: 1,
  }]);
...
async function findProduct(cookie, ref) {
  const ids = await execute(cookie, "product.product", "search", [[["default_code", "=", ref]]]);
  if (!ids.length) throw new Error(`找不到产品货号: ${ref}`);
  const rows = await execute(cookie, "product.product", "read", [ids], { fields: ["id", "list_price"] });
  return rows[0];
}
...
async function createOdooOrder(session) {
  ...
  // 1. 找或创建客户
  const partnerId = await findOrCreatePartner(cookie, email, name);
  ...
  for (const item of items) {
    ...
    try {
      const product = await findProduct(cookie, item.ref);
      orderLines.push([0, 0, {
  ...
  // 4. 创建销售订单
  const soId = await execute(cookie, "sale.order", "create", [{
  ...
  // 5. 确认订单
  await execute(cookie, "sale.order", "action_confirm", [[soId]]);
  console.log(`销售订单已确认`);

  // 6. 创建发票
  await execute(cookie, "sale.order", "_create_invoices", [[soId]]);

  // 7. 取发票 id
  const soData = await execute(cookie, "sale.order", "read", [[soId]], { fields: ["invoice_ids"] });
  ...
  // 8. 确认发票（posted 状态）
  await execute(cookie, "account.move", "action_post", [[invoiceId]]);
  console.log(`发票已确认`);
